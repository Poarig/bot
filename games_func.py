...
def make_up_a_word(update, context):
    ...
    update.message.reply_text("Rearrange the letters correctly \"" + "".join(shuffle_word) + "\" You have 10 seconds")
    ...
    update.message.reply_text(f"I hope you did it.\nCorrect answer is \"{word}\"", reply_markup=games_markup)
...
def random_tongue_twister(update, context):
    games_keyboard = [['/Make_up_a_word'],
                      ...
                      ['/Theory']]
    games_markup = ReplyKeyboardMarkup(games_keyboard, one_time_keyboard=True, resize_keyboard=True)
    update.message.reply_text("This feature is not available yet. Try again later.", reply_markup=games_markup)
